...
function Category() {
    ...
    useEffect(() => {
        fetch(`http://localhost:3004/albums/${albumChooseID}`)
        .then(res => res.json())
        .then(data =>{
            setAlbumInfo(data);
        })
    ...
    if(songs.length != 0 && albums.length != 0 && singers.length != 0 && top100.length != 0 && albumInfo) return (
        <div className="root">
                                            ...
                                            <div className="current-song-info w-3">
                                                <div className="cd" onClick={() => {
                                                    ...
                                                    <div className="cd-thumb"
                                                        style={{backgroundImage: `url(${songs[Number(songId)].image})`}}>
                                                    </div>
                                                </div>
                                                <div className="song-info">
                                                    <div className="song-name">
                                                        {songs[Number(songId)].name}
                                                        <span className={`card ${songs[Number(songId)].vip == "Vip"?'vip-member':'normal-member'}`}>
                                                            {`${songs[Number(songId)].vip == "Vip"? 'Vip': ''}`}
                                                        </span>
                                                    </div>
                                                    <div className="singger-name">
                                                        {singers[songs[Number(songId)].singgerId - 1].name}
                                                    </div>
                                                </div>
                                            </div>
