...
function Category() {
    ...
    const singerChooseLocal = localStorage.getItem('singerChoose');
    const singerChooseID  = JSON.parse(singerChooseLocal);

    useEffect(() => {
        fetch(`http://localhost:3004/singers/${singerChooseID}`)
        .then(res => res.json())
        .then(data =>{
            setSingerInfo(data);
        })
    ...
    if(songs.length != 0 && albums.length != 0 && singers.length != 0 && top100.length != 0 && singerInfo) return (
        <div className="root">
            <div className="container">
                ...
                <div className="main">
                    <div className="home-main">
                        <div className="singer-info-header">
                            <div className="singer-info-image-wrap">
                                <div className="singer-info-image"
                                    style={{backgroundImage: `url(${singerInfo.image})`}}>
                                </div>
                            </div>
                            <div className="singer-info-introduce">
                                <div className="singer-name">{singerInfo.name}</div>
                                <div className="singer-text">{singerInfo.introduce}</div>
                            </div>
                        </div>

                        <div className="list-song">
                            {songs.filter((song, index) => song.singgerId == singerInfo.id).map((song, index) => {
                                return (
